...
function validatePersonalInfo($user) {
    ...
    foreach ($fields as $field) {
        ...
        elseif (strlen($user[$field]) > 20 && $field != "country") {
...
//4
    ...
    $associated_files_ext = bin2hex(random_bytes(16));
    $stmt = $conn->prepare("INSERT INTO creds (first_name, last_name, birthday, country, gender, email, number, personal_links, password, FOEog, YOEog, FOE2, YOE2, FOE3, YOE3, FOE4, YOE4, position_one, position_two, position_three, position_four, personal_background, college_degrees, additional_education, role, salary, vision, notes, associated_files) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
    $stmt->bind_param("ssssssssssisisisissssssssisss",
        ...
        $role, $salary, $vision, $notes,  $associated_files_ext
    );

    $stmt->execute();
    $stmt->close();
    $targetDir = "uploads/".$associated_files_ext."/images";
